...
window.CoachLive = (() => {
  ...
  function utterance(x, open, state) {
    ...
    const translation=tail?'':x.status==='translated'?chinese?`<p class="live-translation" lang="zh-CN">${e(x.chinese)}</p>`:`<details class="live-answer" data-segment="${e(x.id)}" ${open.has(x.id)?'open':''}><summary>看中文</summary><p lang="zh-CN">${e(x.chinese)}</p></details>`:`<p class="live-pending">${x.status==='failed'?'这句翻译失败，英文已保留':state?.translation_status==='unavailable'?'中文翻译暂不可用，英文继续更新':['ended','error','expired','stopped'].includes(state?.status)?'中文尚未完成，原文已保留':'中文稍后出现…'}</p>`;
  ...
  function paint(data) {
    ...
    if(s?.translation_error)messages.push('中文翻译暂不可用：'+s.translation_error+' 原文接收与课后复盘独立运行。');
    const retry=get('#live-retry-translation');if(retry)retry.hidden=!(s?.translation_error&&s.desired!=='stopped'&&!['ended','error','expired','stopped'].includes(s.status));
