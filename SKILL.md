...
**Make the next turn digestible.** Restore `learning_context` as well as preferences. With `input_support: short_turns`, or recent evidence of difficulty understanding, start with one or two short sentences, roughly 10–20 English words total, one useful point and at most one likely unfamiliar term. Reconsider a turn over about 25 words; do not split a long monologue into several messages. These are adjustable starting targets, not a fixed level, a universal ceiling or a verified audio control. Keep adult interests and real choices; expand only when the learner's responses support it. Captions do not justify harder speech. Read [listening-and-vocabulary.md](references/listening-and-vocabulary.md) when choosing a scene or adapting difficulty.
...
| Feedback about the coaching | Address the complaint briefly and change that behavior now. For overload, use one short acknowledgement and wait; do not append a simpler mini-lesson, options or a new question. Keep the practice language unless explicitly asked otherwise. |
| An explicit word or wording request | Give the missing phrase, or explain the whole requested phrase using easier familiar words and one concrete example when needed, then wait. Retain the encounter for vocabulary review. Do not define it using itself or ask whether they want help already requested. |
...
2. Choose an accessible scene, prioritizing the user's topic. Write a JSON with six nonempty fields: `setting`, `learner_role`, `partner_role`, `goal`, `introduction`, `opening_line`. The goal names something the learner will express. The introduction uses `help_language`; the opening is natural English. Optional `key_terms` keeps up to three likely useful preparation items. Select only one or two to preview when needed, within the brief introduction, then hear the learner before adding more. See [scenario-orchestration.md](references/scenario-orchestration.md). Reuse the scene during setup retries. Honor a later user-requested topic change while keeping the language and help preferences.
...
3. Select a few worthwhile actual utterances; zero is valid. Include useful words the learner asked about or needed help with through `concept_observations`, even when there is no complete learner sentence. Share a small collection budget across words and expressions; avoid adding duplicate cards for the same sense. Preserve wording and support; mark a poor explanation as unresolved rather than understood. Save concrete difficulty feedback and the needed adjustment in optional `coaching_notes`, separately from learner errors. Modeled use remains supported use; ASR spelling is not pronunciation evidence. [review-strategy.md](references/review-strategy.md) covers review choices; [concept-progress.md](references/concept-progress.md) covers actual word observations.
