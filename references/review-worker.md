...
- Short temporary turn IDs and a compact auto-review input reduce repeated tokens; the manual finish contract is not copied into every model request. The model selects semantic evidence once. Program code then derives exhaustive omitted-turn and word-check bookkeeping from the selected expression/concept links, restores exact source IDs, and validates the complete archive contract. This keeps every learner turn accounted for without asking the model to emit one mechanical object per turn. Reading help or a concrete omission remains required for priorities. Invalid optional reading annotations are withheld instead of regenerating an otherwise correct review. Semantic selection is still a model judgment.
...
The single model response puts expressions first. As soon as an expression's learner source IDs, exact original quote, usable English and Chinese meaning are complete, code checks those fields against this Voice and publishes up to three provisional suggestions. It does not wait for optional reading metadata. Program-derived full-turn coverage, word checks and evidence validation run before the final commit; only that validated commit changes learning history. Preview is rechecked against a changed source and retained when still source-valid during repair or interruption, and replaced by the saved lesson on success. Timing records first preview separately from full generation.
