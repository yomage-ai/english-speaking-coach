...
The standalone Go companion includes its own runtime, SQLite and TOML reader. No Python, Node.js or Go installation is required. It still needs a current Codex CLI supporting app-server and that user’s existing login. Agent checks the desktop bundled binary before an older PATH CLI; `ENGLISH_COACH_CODEX` may explicitly select a verified binary. No global CLI upgrade or auth-file copy is needed. New live bindings select `gpt-5.6-sol` with `low` for independent translation and teaching, independently of the speaking model. Standalone recovery defaults to the same `gpt-5.6-sol` with `low`; an explicit verified `--model` remains available. Existing explicit binding choices remain unchanged. It calls the hosted model with that user's ChatGPT login and quota; model weights are not bundled or run locally. Availability is verified against that account's model list; an unavailable model produces an error, with no silent fallback to another model. Each lane reuses its authenticated app-server process. Translation reuses an ephemeral conversation for at most eight batches to retain prompt caching, with four recent source segments as explicit context; the previous conversation is unsubscribed before rotation. Teaching also uses a fresh conversation with at most 30 recent source segments. Each batch has up to three segments, normally up to 4,500 characters; original single segments are limited to 12,000 characters.
...
The program enumerates every English span in mixed Chinese/English segments and requires a Chinese meaning for each, including quoted teaching examples. Missing IDs or an English echo fail validation; a proper-name response must preserve the entire unit exactly and is explicitly labeled as retained name text. Name classification is the model’s semantic judgment, never inferred from capitalization. Ordinary words still require Chinese meanings in the model contract. Chinese-only segments are displayed locally even when the model connection is unavailable. During live practice the newest learner intent can still request one written teaching cue; this is separate from copying already-Chinese text. These checks prevent omissions, but do not prove semantic perfection; the original remains available beside the separate Chinese display.

中英混合句按英文片段逐项翻译，中文说明中引用的例句也要有中文句意。缺项或照抄英文不会标记完成，专名可保留原拼写；纯中文原话不等待模型连接；练习中的最新中文意图仍可获得一次英语表达帮助。程序能检查覆盖，不能保证译意永远准确，原话始终保留。
...
Live translation and written teaching use separate, bounded requests. Each complete, source-checked utterance displays as soon as its units arrive; it never waits for the rest of the batch or a teaching response. Teaching receives only the current scene/profile and up to 30 recent bound segments. It shows one exact-source-linked English model or next cue. It hides on changed/new learner wording, older pages and closed practice; suggestions never count as speech or mastery. Short models keep one reading group. Only a new learner intent requests teaching; it is served once, obsolete work is cancelled, and no historical hint queue accumulates. Teaching failures and retries do not hold the caption connection. Both lanes use the existing account quota; this can make a separate teaching request for a learner turn. No model switch or global setting change is implied. A source-text condition prevents an old in-flight translation from overwriting a revised segment.

字幕与书面提示独立请求；一句的所有英文片段校验完成就显示，不等整批或教学完成。教学只处理最新学习者意图，旧请求取消，不积压旧提示。两者使用现有账户额度。提示独立于原话，不是 Voice 已说的话，也不表示已掌握。新发言、原话修订或结束练习后，旧提示隐藏。明确的续接片段保留原文并标注可能的连读词，不把词尾单独当生词翻译；程序不推断发音。
