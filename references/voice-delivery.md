...
Apply the current-intent route at the top of `SKILL.md` before conversation rules. Maintenance requests suspend practice immediately and use the user’s current language, even while Voice remains open; do not close the call unless asked. Use the conversation rules for speech-facing messages inside the exercise. Only English is allowed during practice, including preparation updates, scene introductions, coaching feedback and explanations. Non-English or mixed learner input requires one English restatement and confirmation. Non-English exercise content does not by itself change the practice language; current user instructions still take priority. Follow the open-question rule in `SKILL.md`; never append answer choices. Technical diagnostics and the written review use the user’s current language on the host’s actual visible text surface.

For initial preparation, if the host requires a status before completion, use only `[STATUS] Voice is ready.` Put the single short introduction with place, both roles and learner goal, plus one open role question, in the completed response, then wait. Express these naturally, not as a field-by-field setup report. Do not announce restored state, preferences, level, language mode or setup; do not add a readiness question or “Let’s talk in English” preface. Count status and completion as one spoken turn. On a retry, read the actual recent dialogue and continue if the chosen scene opening was already delivered (an unrelated host greeting does not count). Page delivery and spoken delivery are separate: inspect the exact visible page before describing an open failure; leave unresolved technical status on the written surface. These are ordinary responses to the learner, not instructions to another model.

For a host requiring a `[STATUS]` item for each delegation, use a short English fact grounded in the current meaning. The `[COMPLETE]` item supplies the useful response. Keep STATUS minimal; when COMPLETE already models the intended meaning, do not restate the same meaning in STATUS. Neither should grade the learner, announce a lesson or add a second main question. Do not emit these tags on a host that does not require them.
...
## Startup request and language corrections

Keep the actual user-facing start request explicit about English replies even when the learner uses Chinese; both plugin and Skill default prompts carry this preference. A Skill chip alone may send only its name/path. Do not claim its default prompt was submitted unless it is in the actual user message. A default prompt is a suggested user request, not a Voice configuration API, and does not retrofit an open call. Never fabricate a user turn, send a fake system message, or silently change host/global settings.

A language complaint during the exercise is a coaching control. Own the mistake in one short English sentence, then preserve the pending scene; do not explain that the learner caused the switch. Only an explicit technical investigation leaves practice. For example, “You’re right. I’ll keep speaking English.” is an acknowledgement of the actual preference, not a hidden instruction to another model. If a transcript tail shows drift that has already happened, repair only the next delegated reply; the companion cannot retract speech.
...
Honor the end tool's timing and response contract. The end-call tool is the first operational action on an explicit end: do not open a browser or run review commands first. A host-required STATUS may be brief. Do not delay hanging up to write a review. Where the host supplies an end/tail callback, reconcile the ended Voice immediately through `review-begin`, open the returned exact written review page, and let the registered local worker finish. Do not duplicate generation through finish-review. Ending audio does not finish those actions. Reuse matching saved or pending selections; do not create a second lesson from a repeated tail.
...
When investigating repeated non-English speech, compare unique `realtime_item/transcript_segment` rows with backend `response_item` messages and their timestamps. Distinguish autonomous turns, speech before backend completion, and later paraphrases. Do not count cumulative `transcript_delta` repeats as separate utterances. A compliant backend plus a noncompliant spoken turn is a failed actual-Voice language check.

Inspect current permitted host capabilities before proposing configuration changes: a field in bundled source is not an exposed setting. Some packaged clients disable local Voice overrides; do not patch the app, enable hidden overrides or substitute a paid voice stack as a Skill repair. If no permitted control exists, complete the local Skill corrections and report native speech as unresolved, with concrete evidence for a host fix. 中文：后台英文与实际语音分别验收；没有开放的控制入口时，完成本地可修部分，并明确原生语音问题仍未解决。
