...
Use the conversation rules at the top of `SKILL.md` for all speech-facing messages. Only English is allowed during practice, including preparation updates, scene introductions, coaching feedback and explanations. Non-English or mixed learner input requires one English restatement and confirmation. A request for another language does not authorize a language switch. Follow the open-question rule in `SKILL.md`; never append answer choices. Technical diagnostics and the written review belong on the host's actual visible text surface.
