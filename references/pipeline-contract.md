...
| Transcript → translation | A result belongs to the same full source text that was requested, including after revisions. Chinese spans remain intact. / 译文绑定具体版本原话，保留中文部分。 | Input limits, unsupported text and completed malformed model output affect only their utterance/request. Valid rows survive; a later conflicting/duplicate unit revokes only its affected source version. Each rejected row has at most two total attempts. / 句子问题只影响该句。 |
| Translation scheduling | Complete validated utterances publish individually. While Voice is active, only first attempts enter the single caption lane; rejected rows wait for close/drain or an explicit retry. / 一句校验完立即显示；会中模型通道只处理首轮新句，失败句等结束排空或用户重试。 | A slow or malformed automatic retry cannot start ahead of a later fresh caption. Drain preserves the retry budget, and manual retry remains scoped to the exact run. Previous ephemeral thread subscriptions are released and context stays bounded. / 自动重试不阻塞新字幕；结束补译和手动恢复仍有界且不串场。 |
