...
| Translation scheduling | Captions and written coaching use separate bounded lanes; complete validated utterances publish individually. / 字幕与教学分开，一句校验完立即显示。 | New learner intent cancels obsolete teaching; no stale hint queue. Release previous ephemeral thread subscriptions, retain only bounded explicit context. / 取消过期提示，限制上下文与资源。 |
