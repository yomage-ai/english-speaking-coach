...
| Transcript → translation | A result belongs to the same full source text that was requested, including after revisions. Chinese spans remain intact. / 译文绑定具体版本原话，保留中文部分。 | Input limits, unsupported text and completed malformed model output affect only their utterance/request. Valid rows survive; each rejected row has at most two automatic attempts. / 句子问题只影响该句。 |
