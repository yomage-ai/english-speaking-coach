...
- `ENGLISH_COACH_LATENCY_TEST=1 go test -run TestRealCaptionModelLatencyComparison -v .` compares the caption candidates with identical fictional mixed Chinese/English batches, alternating model order to reduce warm-up bias. It validates every returned utterance before reporting first-batch and total latency. Caption/recovery defaults belong to `defaultCaptionModel`; saved review belongs to `defaultReviewModel`. Do not merge those constants or infer Voice-model behavior from this transport benchmark.
...
Reviewed model transport schemas live in `assets/runtime/contracts.json`; preserve property order for early streaming captions. The companion has no live written-teaching or next-reply prediction schema. Speaking prose is split by purpose/mode in `assets/runtime/speaking.json`; the runtime composes the selected policy. Skill instructions remain the Agent's behavioral entry point. These are explicit local contracts, not a mechanism for overriding another native Voice model.
