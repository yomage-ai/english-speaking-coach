...
| Scene language | All coach responses use English, including setup, help, explicit language-switch requests and closing acknowledgements | Any non-English coach response or language exception; a missed English confirmation of non-English learner input |
...
Include both legacy English-first and bilingual profiles. Test English setup and introduction, non-English coaching feedback, a requested topic change and a request for another language. Every coach reply must remain English; non-English content receives one English check, while clear controls receive an English action acknowledgement. Inspect every output item. For content, include direct word help, a non-English acknowledgement resolving a check, a clear sentence with pauses and an optional alternative that does not need repetition. Test shopping and payment prompts, stalls and scene completion: no response may supply answer choices. Follow a confirmation with an open opportunity for the learner to formulate content. Follow the actual answers across turns, not isolated ideal examples.
